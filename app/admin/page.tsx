...
const nutrientGroups = [
  { title: "Core Indicators", fields: ["ABC3 (mEq/kg)", "ABC4 (mEq/kg)", "Dry Matter (%)", "Moisture (%)"] },
  { title: "Macronutrients", fields: ["Crude Protein (%)", "Crude Fat (%)", "Crude Fiber (%)", "Ash (%)"] },
  { title: "Minerals", fields: ["Calcium (%)", "Total Phosphorus (%)", "Available Phosphorus (%)", "Sodium (%)", "Potassium (%)", "Chloride (%)", "Magnesium (%)"] },
  { title: "Energy", fields: ["ME Poultry (kcal/kg)", "ME Swine (kcal/kg)", "DE (kcal/kg)"] },
...
export default function AdminPage() {
  ...
  const request = async (action: () => Promise<unknown>, success: string) => {
    ...
    catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to save data."); return false; }
  };
  ...
  const saveUser = async () => {
    if (!userDraft) return;
    const ok = await request(() => apiRequest(editingId ? `/api/users/${editingId}` : "/api/users", { method: editingId ? "PUT" : "POST", body: JSON.stringify(userDraft) }), editingId ? "User updated." : "User created.");
    if (ok) closeModal();
  };
  const saveCategory = async () => {
    if (!categoryDraft) return;
    const ok = await request(() => apiRequest(editingId ? `/api/categories/${editingId}` : "/api/categories", { method: editingId ? "PUT" : "POST", body: JSON.stringify(categoryDraft) }), editingId ? "Category updated." : "Category created.");
    if (ok) closeModal();
  };
  const saveIngredient = async () => {
    if (!ingredientDraft) return;
    const ok = await request(() => apiRequest(editingId ? `/api/ingredients/${editingId}` : "/api/ingredients", { method: editingId ? "PUT" : "POST", body: JSON.stringify(ingredientDraft) }), editingId ? "Ingredient updated." : "Ingredient created.");
    if (ok) closeModal();
  };
  ...
  const remove = async (kind: Tab, id: string, label: string) => {
    ...
    await request(() => apiRequest(`/api/${path}/${id}`, { method: "DELETE" }), "Record deleted.");
  };
  ...
  return (
    <main className="admin-shell">
      <aside className="admin-sidebar">
        ...
        <div className="admin-sidebar-footer"><Link className="back-calculator" href="/">← Back to Calculator</Link><button onClick={logout}>Sign Out</button></div>
      </aside>

      <section className="admin-main">
        <header className="admin-topbar"><div><p>Numega Administration</p><h1>{navItems.find((item) => item.id === tab)?.title}</h1></div><div className="admin-top-actions"><button className="admin-logout" onClick={logout} aria-label="Sign out" title="Sign out">↪</button><button className="admin-primary" onClick={openCreate}>＋ Create New</button></div></header>

        <div className="admin-stats">
          <Stat label="Ingredients" value={stats.ingredients} tone="green" />
          <Stat label="Categories" value={stats.categories} tone="amber" />
          <Stat label="Users" value={stats.users} tone="slate" />
        </div>
        ...
        <section className="admin-panel">
          <div className="admin-toolbar">
            ...
            {tab === "ingredients" && <select value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value)}><option value="">All categories</option>{categories.map((category) => <option value={category.id} key={category.id}>{category.name}</option>)}</select>}
          </div>

          {loading ? <div className="admin-empty">Loading PostgreSQL data…</div> : (
            <>
...
const navItems: { id: Tab; label: string; short: string; title: string; icon: string; stat: keyof Stats }[] = [
  { id: "ingredients", label: "Ingredients", short: "Ingredients", title: "Ingredient Management", icon: "▦", stat: "ingredients" },
  { id: "categories", label: "Categories", short: "Categories", title: "Ingredient Categories", icon: "◇", stat: "categories" },
  { id: "users", label: "Users", short: "Users", title: "User Management", icon: "◎", stat: "users" },
];
...
function IngredientList({ ingredients, onEdit, onDelete }: { ingredients: Ingredient[]; onEdit: (item: Ingredient) => void; onDelete: (item: Ingredient) => void }) {
  if (!ingredients.length) return <div className="admin-empty">No ingredients found.</div>;
...
function RowActions({ onEdit, onDelete }: { onEdit: () => void; onDelete: () => void }) { return <div className="row-actions"><button onClick={onEdit} aria-label="Edit">✎</button><button className="danger" onClick={onDelete} aria-label="Delete">×</button></div>; }
