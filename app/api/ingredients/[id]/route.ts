...
export async function PUT(request: NextRequest, context: Context) {
  return handleApi(async () => {
    ...
    if (!text(body["Ingredient Name"]) || !text(body["Category ID"])) {
      return NextResponse.json({ message: "Tên và danh mục nguyên liệu là bắt buộc." }, { status: 400 });
    }
    ...
    if (!result.rowCount) return NextResponse.json({ message: "Không tìm thấy nguyên liệu." }, { status: 404 });
